...
if uploaded_file:
    ...
    # ----- GPA Group Slicer -----
    selected_gpa_group = st.selectbox("🎯 Select GPA Group", options=gpa_labels)

    # ----- Starting Salary Slicer -----
    ...
    # ----- Filter the dataset -----
    filtered_df = df[
        (df["GPA_Group"] == selected_gpa_group) &
        (df["Starting_Salary"] >= salary_range[0]) &
        (df["Starting_Salary"] <= salary_range[1])
    ]
    ...
    st.markdown("""
    **Insight:**  
    This chart shows a filtered view of GPA vs. Starting Salary.  
    There's often a slight upward trend, but other factors likely influence income as well.
    """)
